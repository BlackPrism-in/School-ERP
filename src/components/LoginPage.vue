...
function select(next:Role){role.value=next;[email.value,password.value]=credentials[next];error.value='';recovery.value=false}
function submit(){
 error.value=''
 const expected=credentials[role.value]
 if(email.value.trim().toLowerCase()!==expected[0]||password.value!==expected[1]){error.value=`Use the prefilled ${selected.value.label.toLowerCase()} demo credentials.`;return}
 loading.value=true;setTimeout(()=>{loading.value=false;emit('login',role.value)},650)
}
</script>
